...
import { Guru, Transport, Penggajian } from '@/types';
import { useEffect, useState } from 'react';

interface PenggajianEditProps {
    penggajian: Penggajian;
    gurus: Guru[];
    transports: Transport[];
}

export default function PenggajianEdit({ penggajian, gurus, transports }: PenggajianEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        guru_id: String(penggajian.guru_id),
        periode: penggajian.periode,
        jumlah_sesi: penggajian.jumlah_sesi,
        transport_id: String(penggajian.transport_id),
    });

    const [selectedGuru, setSelectedGuru] = useState<Guru | null>(() => {
        return gurus.find((g) => g.id === penggajian.guru_id) || penggajian.guru || null;
    });

    const [selectedTransport, setSelectedTransport] = useState<Transport | null>(() => {
        return transports.find((t) => t.id === penggajian.transport_id) || penggajian.transport || null;
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('penggajian.update', penggajian.id));
    };
    ...
    return (
        <AuthenticatedLayout>
            <Head title={`Edit Penggajian - ${penggajian.guru?.nama ?? ''}`} />
            ...
            <div className="animate-fade-in page-header flex items-center gap-3 mb-8">
                ...
                <div>
                    <h1 className="page-title text-2xl font-bold text-slate-900 tracking-tight">Edit Data Penggajian</h1>
                    <p className="page-subtitle text-sm text-slate-500">
                        Perbarui data honor dan transport untuk {penggajian.guru?.nama ?? 'Guru'} (Periode {penggajian.periode})
                    </p>
                </div>
            </div>
